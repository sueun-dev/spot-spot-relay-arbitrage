 ...
 *
 * Features:
 * - HMAC-SHA256 + Base64 authentication with passphrase
 * - USDT spot markets
 * - Cross-margin short execution (tdMode:"cross")
 * - WebSocket for real-time data (bbo-tbt channel, 10ms updates)
 * - Balance liability tracking for short positions
 */
class OkxExchange : public ForeignShortExchangeBase {
private:
    ...
    // WebSocket Trade API for low-latency order placement (uses private WS)
    std::unique_ptr<OkxTradeWS> trade_ws_;

    // Private WS for real-time fill data
    ...
    std::atomic<bool> margin_mode_ready_{false};
    std::mutex margin_mode_mutex_;

public:
    OkxExchange(net::io_context& ioc, ExchangeCredentials creds)
        : ForeignShortExchangeBase(Exchange::OKX, MarketType::MarginSpot, "OKX", ioc, std::move(creds)) {
    }
    ...
    // Calls GET /api/v5/asset/currencies (requires auth).
...
private:
    ...
    std::string symbol_to_okx(const SymbolId& symbol) const {
        return std::string(symbol.get_base()) + "-" + std::string(symbol.get_quote());
    }

    // ISO 8601 timestamp for OKX: "2020-12-08T09:08:57.715Z"
    static std::string generate_iso_timestamp();
